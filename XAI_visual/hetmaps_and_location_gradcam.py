...
OUTPUT_DIR = '/mnt/data1/xxx/CHD_1001/Evaluation_Results_VisualV2/DualTokensSize_TS_Heatmap_gradcam_IOU'
...
def create_ensemble_visualization(
    ...
    title = (
        f"Grad-CAM Evaluation purely based on 5x5 Model for: {full_video_filename}\n"
        f"Predicted Class: {pred_label} | Predicted Key Frame: {pred_key_frame} | Final IoU: {iou:.4f}"
    )
    fig.suptitle(title, fontsize=16)

    # Draw individual Grad-CAM heatmaps
    ...
    # Draw fused heatmap and box comparison
    ax_fused = axes[1, 0]
...
def create_original_space_visualization(
    ...
    title = (
        f"Original Space Comparison (Grad-CAM 5x5): {full_video_filename}\n"
...
class GradCAM:
    ...
    def generate_cam(self, model_output, class_idx):
        target_score = model_output["predicted_scores"][0, class_idx]
        ...
        cam = torch.einsum('bsc,bcs->bs', self.activations, weights.transpose(1, 2))
        return F.relu(cam)
...
if __name__ == '__main__':
    ...
    for name, config in MODEL_CONFIGS.items():
        ...
        grad_cams[name] = GradCAM(model=model, target_layer=model.target_spatial_transformer)
    ...
    for disease_type in DISEASE_SUBTYPES:
        ...
        for case_name in tqdm(case_folders, desc=f"Evaluating {disease_type}", unit="case"):
            ...
            try:
                ...
                # =====================================================================
                # 1. Inference: get predictions and key frames
                # =====================================================================
                all_logits_dict = {}
                key_frame_dict = {}
                ...
                # =====================================================================
                # 2. Grad-CAM and IoU calculation
                # =====================================================================
                iou = 0.0
    ...
    # =====================================================================
    # 3. Print and save final report
    # =====================================================================
    print("\n\n" + "="*80)
    print(" " * 15 + "Grad-CAM Spatio-Temporal Localization Evaluation (5x5 Only)")
    print("="*80)
    ...
    for disease_type in DISEASE_SUBTYPES:
        ...
        print(f"  Key Frame Accuracy: {t_acc:.2f}% ({t_hits}/{t_total})")

        print("  Conditional Spatial Alignment (IoU, only if temporal hit):")
    ...
    print(f"Overall Key Frame Accuracy: {overall_t_acc:.2f}% ({overall_t_hits}/{overall_t_total})")
    ...
    # Save JSON report
    report_json_path = os.path.join(OUTPUT_DIR, "final_evaluation_metrics_GradCAM.json")
    with open(report_json_path, 'w', encoding='utf-8') as f:
        json.dump(final_json_report, f, indent=4, ensure_ascii=False)

    print(f"\n🎉 Grad-CAM evaluation finished!")
