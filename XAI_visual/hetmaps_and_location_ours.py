...
OUTPUT_DIR = '/mnt/data1/xxx/CHD_1001/Evaluation_Results_VisualV2/DualTokensSize_TS_Heatmap_ours_IOU'
...
def get_fine_grained_label_from_filename(filename, label_map):
    ...
    if match:
        ...
        if disease_lower == 'normal' or disease_lower == 'noraml':
            disease_upper = 'normal'
        else:
            disease_upper = disease_lower.upper()

        key = f"{disease_upper}_view{view_num}"
...
def create_ensemble_visualization(
    ...
    title = (
        f"Evaluation purely based on 5x5 Model for: {full_video_filename}\n"
    ...
    for i, name in enumerate(model_names):
        ...
        ax.set_title(f'Superimposed Heatmap: {name}', fontsize=14)
        ax.axis('off')

    # Bottom-left: final heatmap (5x5)
    ax_fused = axes[1, 0]
    ...
    if gt_box:
        g_rect = patches.Rectangle((gt_box[0], gt_box[1]), gt_box[2]-gt_box[0], gt_box[3]-gt_box[1],
...
def create_original_space_visualization(
    ...
    title = (
        f"Original Space Comparison (Ours 5x5): {full_video_filename}\n"
...
if __name__ == '__main__':
    ...
    for disease_type in DISEASE_SUBTYPES:
        ...
        for case_name in tqdm(case_folders, desc=f"Evaluating {disease_type}", unit="case"):
            ...
            try:
                true_label_idx = get_fine_grained_label_from_filename(os.path.basename(video_path), POSITIVE_TO_NEGATIVE_PAIR_MAP)
                ...
                # =====================================================================
                # Get model predictions (5x5 only for final evaluation)
                # =====================================================================
                all_logits_dict = {}
                key_frame_dict = {}
                ...
                # =====================================================================
                # Spatial feature extraction & IoU evaluation
                # =====================================================================
                iou = 0.0
                ...
                if is_temporal_hit:
                    ...
                    if gt_box_scaled is not None and final_pred_label_idx > 7:
                        ...
                        for name, model in models.items():
                            with torch.no_grad():
                                outputs = model(input_tensor, None, doppler_masks_by_size[name])
                                attention_vector = outputs["all_layer_attention_scores"][-1][0, final_pred_label_idx, :]

                                res = model.spatial_size
                                heatmap_small = attention_vector.cpu().numpy().reshape(res, res)
                                ...
                                individual_heatmaps_for_vis[name] = heatmap_norm

                        # Use only 5x5 heatmap
    ...
    # =====================================================================
    # Print final evaluation report & save JSON
    # =====================================================================
    print("\n\n" + "="*80)
    print(" " * 20 + "Spatio-Temporal Localization Evaluation Report (Ours - 5x5 Only)")
    print("="*80)
    ...
    for disease_type in DISEASE_SUBTYPES:
        ...
        print(f"  Key Frame Hit Rate (Temporal ACC): {t_acc:.2f}% ({t_hits}/{t_total})")

        print("  Conditional Spatial Alignment (IoU, only on temporal hits):")
        ...
        stats_iou = stats['iou_conditional']
        for thresh in IOU_THRESHOLDS:
            iou_total = stats_iou[thresh]["total"]
            iou_hits = stats_iou[thresh]["hits"]
            iou_acc = (iou_hits / iou_total * 100) if iou_total > 0 else 0.0

            print(f"    IoU > {thresh:.2f}:  Alignment = {iou_acc:6.2f}% ({iou_hits}/{iou_total})")
    ...
    # Overall average
    print("\n" + "-"*80)
    print(" " * 30 + "Overall Average Metrics")
    print("-" * 80)
    ...
    print(f"Overall Key Frame Hit Rate: {overall_t_acc:.2f}% ({overall_t_hits}/{overall_t_total})")
    ...
    print("\n  Overall Conditional Spatial Alignment:")
    for thresh in IOU_THRESHOLDS:
        ...
        print(f"    IoU > {thresh:.2f}:  Alignment = {o_iou_acc:6.2f}%  ({o_iou_hits:>3}/{o_iou_total:>3})")
    ...
    # Save JSON
    report_json_path = os.path.join(OUTPUT_DIR, "final_evaluation_metrics_Ours.json")
    with open(report_json_path, 'w', encoding='utf-8') as f:
        json.dump(final_json_report, f, indent=4, ensure_ascii=False)

    print(f"\n🎉 Our method evaluation completed! All metrics calculated consistently.")
    print(f"📁 Detailed results saved to JSON (ready for paper/Excel):\n -> {report_json_path}")